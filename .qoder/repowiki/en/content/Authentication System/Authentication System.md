...
- Controllers orchestrate requests and responses for authentication endpoints.
- Services encapsulate business logic for tokens, credentials, and user persistence.
- Middleware validates access tokens and refresh tokens and parses tokens from cookies.
...
- Entities model persisted user and refresh token data.
...
The authentication flow integrates route handlers, middleware, controllers, and services. Access tokens are validated using JWKS-based RS256, while refresh tokens are validated using HS256 with revocation checks against the database.
...
ROUTES --> MW2["parseToken.js"]
ROUTES --> MW3["validateRefresh.js"]
...
## Conclusion
The authentication system employs a robust dual-token strategy with RS256 access tokens and HS256 refresh tokens, leveraging JWKS-based validation and database-backed revocation. It integrates bcrypt for secure password handling, cookie-based session-like management, and a clear token lifecycle with rotation and logout. By following the recommended security practices and troubleshooting steps, the system provides a secure and maintainable foundation for user authentication.
...
- Use HTTPS in production to protect cookies and JWKS endpoints.
- Set secure flag on cookies in production environments.
- Rotate refresh tokens on every use and implement short refresh token TTLs.
- Monitor and audit token issuance and revocation events.
- Regularly rotate signing keys and update JWKS accordingly.
